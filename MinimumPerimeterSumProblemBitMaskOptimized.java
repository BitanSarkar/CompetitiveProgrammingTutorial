import java.util.Arrays;
import java.util.Scanner;
public class MinimumPerimeterSumProblemBitMaskOptimized {
    public static void main(String[] args){
        Scanner sc = new Scanner(System.in);
        //Enter number of groups
        int n = sc.nextInt();
        sc.nextLine();
        double[] x = new double[3*n];
        double[] y = new double[3*n];
        int used = (1<<(3*n)) - 1;
        double[] usedMap = new double[used+1];
        Arrays.fill(usedMap, Double.MAX_VALUE);
        //Enter all 3*n points
        for(int i = 0; i<3*n; i++){
            String[] ip = sc.nextLine().split("\\s+");
            x[i] = Double.parseDouble(ip[0]);
            y[i] = Double.parseDouble(ip[1]);
        }
        long init = System.currentTimeMillis();
        System.out.println(minPerimeterSum(x, y, usedMap, used, 3*n));
        long end = System.currentTimeMillis();
        System.out.println("Execution time : " + (end-init) + "ms");
        sc.close();
    }

    
    private static double perimeter(double x0,double x1,double x2,double y0,double y1,double y2){
        return  Math.sqrt((x0-x1)*(x0-x1) + (y0-y1)*(y0-y1)) + 
                Math.sqrt((x0-x2)*(x0-x2) + (y0-y2)*(y0-y2)) + 
                Math.sqrt((x2-x1)*(x2-x1) + (y2-y1)*(y2-y1));
    }

    private static double minPerimeterSum(double[] x, double[] y, double[] usedMap, int used, int total) {
        if(x==null || y==null) return Double.MAX_VALUE;
        int usedCtr = Integer.bitCount(used);
        int idx1 = 0;
        int idx2 = 0;
        int idx3 = 0;
        int innerCtr = 0;
        for(int i = 0; i<total; i++) {
            if(((used >> i) & 1) == 1) {
                if(innerCtr == 0) idx1 = i;
                if(innerCtr == 1) idx2 = i;
                if(innerCtr == 2) {
                    idx3 = i;
                    break;
                }
                innerCtr++;
            }
        }
        if(usedCtr<=2) return Double.MAX_VALUE;
        if(usedCtr==3) {
            return perimeter(x[idx1], x[idx2], x[idx3], y[idx1], y[idx2], y[idx3]);
        }
        Integer key = used;
        if(usedMap[key]!=Double.MAX_VALUE) return usedMap[key];
        double minPerimeter = Double.MAX_VALUE;
        for(int i = 0; i<total; i++) {    
            for(int j = i+1; j<total; j++) {
                for(int k = j+1; k<total; k++) {                    
                    int ithBit = (used>>i)&1;
                    int jthBit = (used>>j)&1;
                    int kthBit = (used>>k)&1;
                    if(ithBit == 1 && jthBit == 1 && kthBit == 1 && i!=j && k!=j && k!=i){
                        used = used & ~(1<<i) & ~(1<<j) & ~(1<<k);
                        double currentPerimeter = perimeter(x[i], x[j], x[k], y[i], y[j], y[k]) + minPerimeterSum(x, y, usedMap, used, total);
                        minPerimeter = currentPerimeter < minPerimeter ? currentPerimeter : minPerimeter;
                        used = used | (1<<i) | (1<<j) | (1<<k);
                    }
                }
            }
        }
        usedMap[key] = minPerimeter;
        return minPerimeter;
    }
}